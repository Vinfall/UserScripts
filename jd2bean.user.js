...
(() => {
    const debitAccount = {
        ...
        // 食物
        '矿泉水|芬达|雪碧|可乐|红茶|海之盐|阿萨姆|送水|怡宝|饮料|美年达|奶茶|[鲜牛]奶|饮品|咖啡|cafe|拿铁|烧仙草':
    ...
    function chooseExpenseAccount(goodsName) {
        for (const debitAccountKey in debitAccount) {
            const regex = new RegExp(debitAccountKey);
            const re = goodsName.match(regex);
            if (re != null) {
    ...
    if ($('.td-void.order-tb').length > 0) {
        setTimeout(() => {
            const orders = Array.from($('.order-tb tbody[id*="tb-"]'))
                .filter((order) => {
                ...
                .map((order) => {
                    const { date, time } = splitDateTime(getText('.dealtime', order));
...
`.trim();
    ...
    if ($('.goods-total').length > 0) {
        ...
        const details = Array.from($('tr[class*="product"]'))
            .map((product) => {
